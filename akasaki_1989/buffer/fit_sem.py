...
def print_parms_to_file(fitParms,types,thresh,s2):
    ...
    lines[15+adder]="**Threshold Value:** {:0.4f}\\\n".format(thresh)
...
def gausIntersect(fitParms,types,debug=True):
    ...
    if types[0] != curve_types[2]:
        ...
        if intersect < fitParms[1] or intersect > fitParms[4]:
            intersect = res2
        ...
        if intersect < fitParms[1] or intersect > fitParms[5]:
            intersect = res1
            print("HERE"+str(intersect))
...
def fit_pixel_hist(imageFile,types,paramsToFit,bnds,range_list,debug=True):
    ...
    high_t = np.ceil(threshold_value)
    low_t = np.floor(threshold_value)
    remainder = threshold_value - low_t
    


    ret,thresh1 = cv2.threshold(gray,high_t,255,cv2.THRESH_BINARY)
    #nzCount = cv2.countNonZero(thresh1)
    nzCount = sum(y1[int(high_t)-1:])
    imgSize = thresh1.size
    #s2 = nzCount/float(imgSize)
    s2_highT = nzCount/float(sum(y1))
    print(f"S Upper T:{s2_highT} at pixel {high_t}")

    ret,thresh1 = cv2.threshold(gray,low_t,255,cv2.THRESH_BINARY)
    #nzCount = cv2.countNonZero(thresh1)
    nzCount = sum(y1[int(low_t)-1:])
    imgSize = thresh1.size
    #s2 = nzCount/float(imgSize)
    s2_lowT = nzCount/float(sum(y1))
    print(f"S Lower T:{s2_lowT} at pixel {low_t}")
    s2 = remainder*s2_highT + (1-remainder)*s2_lowT
    threshold_value = remainder*high_t + (1-remainder)*low_t
...
if __name__ == "__main__":
    switch = sys.argv[1]
    if switch == "init":
        preview_hist("sem.png")
    elif switch == "test":
        types,parms,bnds,range_list=readParms("parms.dat")
        fit_pixel_hist("sem.png",types,parms,bnds,range_list)
    elif switch == "final":
        types,parms,bnds,range_list=readParms("parms.dat")
        fit_pixel_hist("sem.png",types,parms,bnds,range_list,debug=False)
    else:
